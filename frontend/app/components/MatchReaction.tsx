...
function Ring({ tile }: { tile: SolitaireTile }) {
  ...
  return <mesh ref={ref} position={[tile.x * 0.88, 0.3 + tile.layer * 0.52, tile.z * 1.16]} rotation={[-Math.PI / 2, 0, 0]}>
    <ringGeometry args={[0.28, 0.32, 24]} /><meshBasicMaterial color="#b6ffe2" transparent depthWrite={false} />
  </mesh>;
}
export default function MatchReaction({ tiles }: { tiles: SolitaireTile[] }) {
  return <group>{tiles.map(tile => <Ring key={tile.id} tile={tile} />)}</group>;
}